...
- Improved Observability declarations with a Mastra attachment example and directions to related documentation bundled in core. ([#23489](https://github.com/mastra-ai/mastra/pull/23489))

- Fixed built-in storage exporters continuing to write in Mastra Platform deployments. Other exporters remain enabled. ([#23555](https://github.com/mastra-ai/mastra/pull/23555))
...
- Model step and model inference spans now use the `ModelStepInput` type from `@mastra/core` for their input. No change to what the spans record. ([#23141](https://github.com/mastra-ai/mastra/pull/23141))
...
- Skip impossible JSON prefixes in SensitiveDataFilter to avoid repeated parse exceptions for serialization markers while preserving valid embedded-JSON redaction. ([#23314](https://github.com/mastra-ai/mastra/pull/23314))
...
- Fixed incomplete traces when a model errors mid-generation. The open `MODEL_STEP` and `MODEL_INFERENCE` spans are now closed with the error instead of being left dangling, so error traces are complete. ([#18083](https://github.com/mastra-ai/mastra/pull/18083))
