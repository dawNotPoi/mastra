...
- Fixed `@mastra/nestjs` to run the same auth pipeline as the other server adapters. Cookie-based sessions now authenticate, custom routes registered with `registerApiRoute()` are served (honoring `requiresAuth`), and `mapUserToResourceId` is applied to the request context. Fixes #24964. ([#25027](https://github.com/mastra-ai/mastra/pull/25027))
...
- Align the NestJS adapter with the shared server route contract for dynamic routes, body validation, and HTTP exceptions. ([#24254](https://github.com/mastra-ai/mastra/pull/24254))
...
- Fixed Studio client-type detection to read the x-mastra-client-type header via request.headers instead of the Express-only request.get(), matching how the adapter reads headers everywhere else. ([#18793](https://github.com/mastra-ai/mastra/pull/18793))
